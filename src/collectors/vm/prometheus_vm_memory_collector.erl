...
collect_metrics(erlang_vm_memory_processes_bytes_total, Memory) ->
  gauge_metrics([{[{usage, used}], proplists:get_value(processes_used, Memory)},
                 {[{usage, free}], proplists:get_value(processes, Memory) - proplists:get_value(processes_used, Memory)}]);
collect_metrics(erlang_vm_memory_system_bytes_total, Memory) ->
  gauge_metrics([{[{usage, atom}], proplists:get_value(atom, Memory)},
...
collect_metrics(erlang_vm_memory_atom_bytes_total, Memory) ->
  gauge_metrics([{[{usage, used}], proplists:get_value(atom_used, Memory)},
                 {[{usage, free}], proplists:get_value(atom, Memory) - proplists:get_value(atom_used, Memory)}]);
