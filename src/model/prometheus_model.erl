...
d_field_Bucket_upper_bound(<<0:48, 240, 255,
			     Rest/binary>>,
			   Z1, Z2, F1, _) ->
    dfp_read_field_def_Bucket(Rest, Z1, Z2, F1,
			      '-infinity');
...
d_field_Gauge_value(<<_:48, 15:4, _:4, _:1, 127:7,
		      Rest/binary>>,
		    Z1, Z2, _) ->
    dfp_read_field_def_Gauge(Rest, Z1, Z2, nan);
...
-dialyzer({nowarn_function,v_msg_Bucket/2}).
...
-dialyzer({nowarn_function,v_msg_Histogram/2}).
...
-dialyzer({nowarn_function,v_msg_Untyped/2}).
...
-dialyzer({nowarn_function,v_msg_Quantile/2}).
...
-dialyzer({nowarn_function,v_msg_Summary/2}).
...
-dialyzer({nowarn_function,v_msg_Counter/2}).
...
-dialyzer({nowarn_function,v_msg_Gauge/2}).
...
-dialyzer({nowarn_function,v_msg_LabelPair/2}).
...
-dialyzer({nowarn_function,v_msg_MetricFamily/2}).
